...
fn github_payload_selects_platform_installer() {
    let release = release_from_github_payload(&json!({
        "tag_name": "v1.0.9",
        "html_url": "https://github.com/BigPizzaV3/JanCode/releases/tag/v1.0.9",
        "body": "fixes",
        "assets": [
            {"name": "source.zip", "browser_download_url": "https://example.test/source.zip"},
            {"name": "jancode-manager.exe", "browser_download_url": "https://example.test/manager.exe"},
            {"name": "JanCode_1.0.9_x64-setup.exe", "browser_download_url": "https://example.test/setup.exe"},
            {"name": "JanCode_1.0.9_x64.dmg", "browser_download_url": "https://example.test/app.dmg"}
        ]
    }))
    .unwrap();
    ...
    if cfg!(windows) {
        assert_eq!(
            ...
            Some("JanCode_1.0.9_x64-setup.exe")
        );
    } else if cfg!(target_os = "macos") {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("JanCode_1.0.9_x64.dmg")
        );
    } else {
...
fn latest_json_payload_selects_platform_installer_without_github_api_shape() {
    let release = release_from_latest_json_payload(&json!({
        "version": "v1.1.6",
        "url": "https://github.com/BigPizzaV3/JanCode/releases/tag/v1.1.6",
        "body": "静态更新描述",
        "assets": [
            {"name": "source.zip", "url": "https://example.test/source.zip"},
            {"name": "JanCode-1.1.6-windows-x64-setup.exe", "url": "https://example.test/setup.exe"},
            {"name": "JanCode-1.1.6-macos-x64.dmg", "url": "https://example.test/app.dmg"}
        ]
    }))
    .unwrap();
    ...
    if cfg!(windows) {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("JanCode-1.1.6-windows-x64-setup.exe")
        );
    } else if cfg!(target_os = "macos") {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("JanCode-1.1.6-macos-x64.dmg")
        );
    } else {
        assert_eq!(release.asset_name.as_deref(), None);
    }
...
fn asset_selection_prefers_current_platform_artifacts() {
    let assets = vec![
        (
            "JanCode.zip".to_string(),
            "https://example.test/source.zip".to_string(),
        ),
        (
            "jancode-manager.exe".to_string(),
            "https://example.test/manager.exe".to_string(),
        ),
        (
            "JanCode_1.0.9_x64-setup.exe".to_string(),
            "https://example.test/setup.exe".to_string(),
        ),
        (
            "JanCode_1.0.9_x64.dmg".to_string(),
    ...
    if cfg!(windows) {
        ...
        assert_eq!(selected.name, "JanCode_1.0.9_x64-setup.exe");
    } else if cfg!(target_os = "macos") {
        let selected = select_update_asset(&assets).unwrap();
        assert_eq!(selected.name, "JanCode_1.0.9_x64.dmg");
    } else {
...
fn asset_selection_distinguishes_x64_and_arm64_macos_dmgs() {
    ...
    let assets = vec![
        (
            "JanCode-1.2.17-macos-arm64.dmg".to_string(),
            "https://example.test/app-arm64.dmg".to_string(),
        ),
        (
            "JanCode-1.2.17-macos-x64.dmg".to_string(),
    ...
    if cfg!(target_os = "macos") {
        ...
        let expected = match std::env::consts::ARCH {
            "x86_64" => "JanCode-1.2.17-macos-x64.dmg",
            "aarch64" => "JanCode-1.2.17-macos-arm64.dmg",
