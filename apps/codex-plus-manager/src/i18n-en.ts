// English translations for the JanCode manager UI, keyed by the original
...
export const EN_PLAIN: Record<string, string> = {
  "技能中心": "Skills Center",
  "内置技能库与触发词匹配，按任务自动启用技能": "Built-in skill library with trigger matching; enables skills by task",
  "技能库": "Skill library",
  "触发测试": "Trigger tester",
  "专家模式：开": "Expert mode: on",
  "专家模式：关": "Expert mode: off",
  "搜索技能名称、说明或触发词": "Search skill name, summary or trigger",
  "已启用": "Enabled",
  "启用": "Enable",
  "进阶": "Advanced",
  "没有匹配的技能。": "No matching skills.",
  "试试打开专家模式查看进阶技能。": "Try turning on expert mode to see advanced skills.",
  "输入一段任务描述，看看会自动命中哪些技能。命中强度按触发词的数量与具体程度累加，越靠前表示越相关。": "Type a task description to see which skills it triggers. Score accumulates over the number and specificity of matched triggers; higher means more relevant.",
  "例如：帮我做一次代码审查，顺便看看这段 SQL 的索引有没有问题": "e.g. Do a code review, and also check whether this SQL uses its indexes well",
  "还没有输入内容。": "Nothing entered yet.",
  "没有命中任何技能。这是正常的——技能只在任务确实相关时才会启用。": "No skill matched. That is expected — skills only activate when the task is genuinely relevant.",
  "强度": "Score",
  "命中触发词：": "Matched triggers: ",
  "主导航": "Main navigation",
  "工作区": "Workspace",
  "扩展": "Extensions",
  "系统": "System",
  ...
  "JanCode 导入": "JanCode import",
  "JanCode 版本": "JanCode version",
  ...
  "运行状态": "Runtime status",
  "配置保存在 JanCode，实时操作通过本机回环 CDP 执行": "Configuration stays in JanCode; live operations use the local loopback CDP connection",
  ...
  "自定义图片会被导入 JanCode 托管目录；主题字段与目标项目 theme.json 对齐":
    "Custom images are imported into JanCode managed storage; theme fields match the source theme.json.",
  "我的主题": "My themes",
  ...
  "Watcher 用于保持 JanCode 接管状态": "The watcher keeps JanCode in control",
  ...
  "保存到 JanCode state，不改写 Zed settings。": "Saved to JanCode state, does not modify Zed settings.",
  ...
  "卸载时移除 JanCode 托管数据": "Remove JanCode managed data on uninstall",
  ...
  "在 Codex 页面显示可拖动的后续建议浮层；建议由单独配置的 Stepwise API 生成。启停后需重启 JanCode 生效。":
    "Shows a draggable follow-up suggestion overlay in the Codex page. Suggestions are generated by a separate Stepwise API. Restart JanCode after changing this setting.",
  ...
  "导入 JanCode 供应商": "Import JanCode provider",
  ...
  "开启": "On",
  "开启后，通过 JanCode 启动 Codex 前自动整理一次旧对话的归属标记。":
    "When on, tidies up ownership tags of old conversations once before launching Codex through JanCode.",
  ...
  "使用 JanCode 内置快照补齐远端插件，API 模式也可显示和安装 Product Design 插件。":
    "Use the embedded JanCode snapshot to add remote plugins, so API mode can also show and install the Product Design plugin.",
  ...
  "未发现本地缓存；点击按钮会从 JanCode 内置快照释放并注册，无需官方账号预缓存。":
    "Local cache not found. Click the button to extract and register the embedded JanCode snapshot; no official-account pre-cache is required.",
  "未知": "Unknown",
  "未知作者": "Unknown author",
  "未知来源": "Unknown source",
  ...
  "检测到来自网页的供应商配置导入请求，确认后会写入本机 JanCode 管理工具。":
    "A provider config import request from the web was detected; confirming writes it to this machine's JanCode manager.",
  "概览": "Overview",
  ...
  "正常": "OK",
  "此上游会通过本地 127.0.0.1:57321 转成 Responses API，需要从 JanCode 启动 Codex。":
    "This upstream is converted to the Responses API via local 127.0.0.1:57321, which requires launching Codex from JanCode.",
  ...
  "维护 JanCode 自己的远程项目最近列表。": "Maintain JanCode's own recent list of remote projects.",
  "编辑": "Edit",
  "编辑后保存列表，再切换模式时会使用新配置": "After editing, save the list; the new config is used the next time you switch modes",
  "编辑扩展项": "Edit entry",
  "缺失": "Missing",
  "缺少 JanCode 静默启动快捷方式时可在安装维护页修复。": "When the JanCode silent launch shortcut is missing, repair it on the Install & Maintenance page.",
  ...
  "仅在当前供应商启用时生效；精确匹配模型名并使用目标供应商的 URL 与 Key。目标必须是 Responses API，且需要从 JanCode 启动。":
    "Applies only while this provider is active. It exactly matches the model name and uses the target provider's URL and key. The target must use the Responses API, and Codex must be launched through JanCode.",
...
export const EN_TEMPLATE: Record<string, string> = {
  ...
  "Grok 里所有由 JanCode 管理的模型表会被供应商「{0}」的模型列表整体替换（[ui]、web_search 等未管理字段保留）。原配置会先备份。":
    "Every model table managed by JanCode in Grok will be replaced wholesale by the model list of provider \"{0}\" (unmanaged fields such as [ui] and web_search are kept). The previous config is backed up first.",
  ...
  "{0} 个": "{0}",
  "{0} 个 JanCode 可识别项目，默认策略：{1}": "{0} project(s) recognized by JanCode, default strategy: {1}",
  "{0} 个供应商": "{0} provider(s)",
  ...
  "检测到代理环境变量：{0}。请清理后重新启动 JanCode。":
    "Proxy environment variables detected: {0}. Remove them and restart JanCode.",
